...
local function render_type(ty, type_names)
	...
	elseif kind == "map" then
		return "HashMap<String, " .. render_type(ty.value, type_names) .. ">"
	elseif kind == "nullable" then
		return "Option<" .. render_type(ty.of, type_names) .. ">"
	end
	error("unknown type kind: " .. tostring(kind))
end

local function type_uses_map(ty)
	local kind = ty.kind
	if kind == "map" then
		return true
	elseif kind == "array" then
		return type_uses_map(ty.item)
	elseif kind == "nullable" then
		return type_uses_map(ty.of)
	end
	return false
end

local function document_uses_map(document)
	if type_uses_map(document.root) then
		return true
	end
	for _, named in ipairs(document.types) do
		for _, field in ipairs(named.fields) do
			if type_uses_map(field.ty) then
				return true
			end
		end
	end
...
return {
	...
	render = function(document)
		...
		if document_uses_map(document) then
			out[#out + 1] = "use std::collections::HashMap;\n"
		end
		out[#out + 1] = "\n"
