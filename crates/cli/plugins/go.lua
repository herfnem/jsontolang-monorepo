...
local function is_go_value_type(ty)
	local kind = ty.kind
	return kind == "bool"
		or kind == "integer"
		or kind == "unsigned_integer"
		or kind == "float"
		or kind == "string"
		or kind == "named"
end
...
local function render_type(ty)
	...
	elseif kind == "map" then
		return "map[string]" .. render_type(ty.value)
	elseif kind == "nullable" then
		local inner = render_type(ty.of)
		if is_go_value_type(ty.of) then
			return "*" .. inner
		end
		return inner
	end
...
local function render_field_type(field)
	...
	if field.optional and ty:sub(1, 1) ~= "*" then
		return "*" .. ty
	end
	return ty
end
